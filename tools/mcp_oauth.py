...
def _make_callback_waiter(port: int, cimd_url: str | None = None, timeout: float = 300.0):
    ...
    async def _wait():
        dashboard_flow = get_dashboard_oauth_flow()
        if dashboard_flow is not None:
            # Dashboard flow speaks the legacy tuple; normalize to one shape. The ``iss`` form is
            # required for any authorization server that advertised
            # ``authorization_response_iss_parameter_supported`` (RFC 9207) — Cloudflare, Atlassian,
            # Notion, GitHub MCP, etc. Without it the SDK raises ``Authorization response missing iss
            # parameter advertised by the authorization server`` and the flow never reaches token exchange.
