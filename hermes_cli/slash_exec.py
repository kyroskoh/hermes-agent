...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    lines = [t("gateway.help.header"), *gateway_help_lines(allowed_commands=allowed_commands)]
    skill_cmds = _skill_commands()
    try:
        # If user has a gated command list, only show skills they are allowed to run.
