...
def _scan_profile_flag(argv: list) -> tuple:
    """Find -p/-pro/-agent/--profile/--profile= in argv -> (name, tokens_consumed, index).
    ...
    while i < len(argv):
        ...
        if arg == "--" or (arg == "--args" and _inside_mcp_add_args(argv, i)):
            break
        if arg in {"--profile", "-p", "-pro", "-agent"} and i + 1 < len(argv):
...
def _scan_personality_flag(argv: list) -> tuple:
    """Find -p2/-persona/--personality/--personality= in argv -> (name, tokens_consumed, index).

    Same broad-scan rules as ``_scan_profile_flag``. CLI alias family for /personality
    (overlay): -p2, -persona. (/r stays gateway-slash-only -- it would collide with --resume.)
    """
    from hermes_cli._parser import top_level_value_flag_sets
    ...
    while i < len(argv):
        ...
        if arg == "--" or (arg == "--args" and _inside_mcp_add_args(argv, i)):
            break
        if arg in {"--personality", "-p2", "-persona"} and i + 1 < len(argv):
            return argv[i + 1], 2, i
...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and --personality/-p2 and set HERMES_HOME / the personality overlay
    before imports."""
    # Personality overlay is independent of the profile resolution below, so it is scanned, applied,
    # and stripped from sys.argv FIRST -- via the single-sanctioned write path so the config key is
    # set exactly the same way the /personality slash command would set it. Empty/neutral names are
    # forwarded as-is and resolve to the cleared overlay. Doing this before any profile-scan
    # early-return (below) means -p2/-persona work even when no -p/--profile flag is present, and
    # stripping it before the profile flag is (re-)scanned keeps that scan's argv indices valid.
    personality_name, personality_consume, personality_index = _scan_personality_flag(sys.argv[1:])
    if personality_name is not None:
        try:
            ...
            persist_personality(normalize_personality_name(personality_name))
        ...
        if personality_consume > 0 and personality_index is not None:
            start = personality_index + 1  # +1 because argv is sys.argv[1:]
