...
def _state_db_health(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str) -> None:
    ...
    try:
        ...
        if _write_reason is not None:
            ...
            # A corrupt page can surface while exercising an FTS trigger, but that does not make it
            # an FTS-only fault. In-place FTS surgery is unsafe when SQLite itself reports SQLITE_CORRUPT.
            if is_malformed_db_error(sqlite3.DatabaseError(_write_reason)):
                # Dispatch to the structural branch below, which deliberately does not repair in place.
        ...
        from hermes_state import is_malformed_db_error, is_malformed_schema_error
        if not is_malformed_db_error(e):
            return check_warn(f"{_DHH}/state.db exists but has issues: {e}")
        if not is_malformed_schema_error(e):
            # SQLite itself reports structural corruption (not just a malformed sqlite_master row) —
            # in-place repair is unsafe; the operator must restore from a verified backup.
