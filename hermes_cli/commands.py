...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("personality", "Set a predefined personality", "Configuration",
               args_hint="[name]", argument_mode="options", aliases=("p2", "persona", "r")),
    ...
    CommandDef("reload-lookups", "Reload cached gateway lookups (commands, skills, profile routing) without restart",
               "Tools & Skills", aliases=("reload_lookups",),
               gateway_only=True, busy_policy="dispatch", execute="reload"),
...
def gateway_help_lines(allowed_commands: set[str] | frozenset[str] | None = None) -> list[str]:
    ...
    for cmd in COMMAND_REGISTRY:
        ...
        alias_parts = [f"`/{a}`" for a in cmd.aliases
                       ...
                       and (allowed_commands is None or a in allowed_commands)]
