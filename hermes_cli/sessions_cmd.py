...
def _cmd_smart_recover(args):
    """Fail-closed production recovery: classify the default state.db (or, with --auto, recover it
    in a detached unit). Delegates to the ``hermes-state-db-*`` scripts rather than opening the DB
    in this process — structural corruption must never be touched by a live handle."""
    import subprocess
    ...
    if not smart.is_file() or not recover.is_file():
        print(f"Error: Hermes state-db recovery scripts are not installed at {smart.parent}.")
        return 1
    if not getattr(args, "auto", False):
        # The smart script's check path is deliberately non-mutating.
        return subprocess.run([str(smart)], check=False).returncode
    # The wrapper uses systemd-run and returns immediately. It must be a separate process because it
    # may gracefully terminate this REPL after collecting forensic WAL/SHM descriptors.
...
_PRE_DB_HANDLERS = {
    ...
    "smart-recover": _cmd_smart_recover,
}
_DB_HANDLERS = {
