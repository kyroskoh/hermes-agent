...
from agent.redact import redact_sensitive_text as _redact_tokens
    ...
    def _async_writer_loop(self) -> None:
        ...
        while True:
            try:
                ...
                err_msg = (
                    f"{_redact_tokens(str(first_error), force=True)}" if first_error is not None
                    else "unknown error"
                )
                logger.warning(
    ...
    def _on_write_giveup(self, session_key: str, item: Any, last_err: Exception | None) -> None:
        ...
        if not already_alerted:
            ...
            logger.critical(
                ...
                _redact_tokens(last_err_str, force=True)[:300],
            )
