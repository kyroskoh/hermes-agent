...
CONFIG_SCHEMA = ProviderConfigSchema(
    ...
    fields=(
        ...
        _field("writeRetryMax", "Write retry max attempts", KIND_NUMBER,
               "Maximum attempts to retry a failed async Honcho write before giving up. With the default "
               "initial backoff of 2s, this gives about 4 minutes of recovery before paging the operator.",
               default="8", placeholder="8", group="Message writing"),
        _field("writeRetryInitialBackoff", "Write retry initial backoff (s)", KIND_NUMBER,
               "Initial wait before the first async write retry. Each subsequent retry doubles the wait, "
               "capped at 60s.",
               default="2.0", placeholder="2.0", group="Message writing"),
        # — Dialectic —
