...
def _behavior_fields(look: _HostLookup, explicitly_configured: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "write_retry_max": look.parsed("writeRetryMax", int, 8),
        "write_retry_initial_backoff": look.parsed("writeRetryInitialBackoff", float, 2.0),
...
class HonchoClientConfig:
    ...
    # Async write retry policy: on a Honcho sync failure (network blip, container restart, 5xx),
    # the async writer retries with exponential backoff (doubling, capped at 60s) up to
    # write_retry_max times before giving up and emitting a CRITICAL log line tagged with the
    # session key. Default 8 retries starting at 2s (~4 min total recovery window) — long enough
    # to ride out a typical Honcho container restart, short enough a stuck session doesn't
    # accumulate forever.
    write_retry_max: int = 8
    write_retry_initial_backoff: float = 2.0  # seconds; doubles per retry, capped at 60s
