...
class PlatformConfig:
    ...
    _TYPED_KEYS = frozenset({
        "enabled", "token", "api_key", "home_channel", "home_channels", "reply_to_mode", "channel_overrides",
...
class GatewayConfig:
    ...
    def get_home_channels(self, platform: Platform) -> List[HomeChannel]:
        ...
        `home_channels`. Deduplication happens at load time for configs
        loaded via `from_dict`, but a caller that sets `.home_channel`
        directly (e.g. a test, or code assembling a config in-process
        rather than from disk) never goes through that merge — so this
        also dedupes here by (chat_id, thread_id) to keep both paths safe.
        ...
        if not config:
            return []
        result: List[HomeChannel] = []
        seen: set = set()
        if config.home_channel:
            key = (str(config.home_channel.chat_id), str(config.home_channel.thread_id) if config.home_channel.thread_id else None)
            seen.add(key)
            result.append(config.home_channel)
        for hc in config.home_channels:
            key = (str(hc.chat_id), str(hc.thread_id) if hc.thread_id else None)
            if key in seen:
                continue
            seen.add(key)
