...
class GatewayNotificationsMixin:
    ...
    def _home_channel_transports(self):
        ...
        Multi-home broadcast: an operator reachable on more than one chat (e.g. two WhatsApp LIDs, or
        a Telegram home + a Discord admin thread) gets every configured home. ``home_channels`` already
        includes the singular ``home_channel`` for back-compat, so single-home configs behave exactly
        as before.
        """
        from gateway.delivery import resolve_delivery_transport
