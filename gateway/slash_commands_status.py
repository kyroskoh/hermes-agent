...
class GatewayStatusCommandsMixin:
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # FORK: kyroskoh/hermes-agent — query ALL configured providers (Nous, Codex,
        # Anthropic, OpenRouter, etc.) and render their current quotas, limits,
        # balances, and reset/refresh dates. Gates on "has stored credentials", NOT
        # the currently active inference provider, so a Nous-credentialled user
        # running inference elsewhere still sees their balance. Fail-open: never
        # break /usage. Supersedes the single-provider account_snapshot fetch above
        # when it returns anything.
        from agent.account_usage import all_configured_account_usage_lines
        all_quota_lines = await _quiet(
            lambda: asyncio.to_thread(all_configured_account_usage_lines, markdown=True), []
        )
        if all_quota_lines:
            account_lines = all_quota_lines

        # Token usage analytics across all providers & models (last 30d).
        providers_token_lines: list[str] = []
        try:
            from hermes_cli.web_routers.analytics import _aggregate_by_provider, _get_usage_analytics

            analytics = await asyncio.to_thread(_get_usage_analytics, 30, None)
            by_provider = _aggregate_by_provider(analytics.get("by_model") or [])
            if by_provider:
        ...
        if account_lines or providers_token_lines:
            return _with_account_blocks([])
