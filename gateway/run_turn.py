...
class GatewayTurnMixin:
    ...
    def _resolve_session_agent_runtime(
        ...
        #
        # BUT: when the caller has a known global default model in user_config, prefer it —
        # otherwise multiplexed chats whose config lookup returned empty would silently fall to
        # whichever provider's first catalog model is cheapest (commonly kimi-k3 on opencode-go,
        # which has a flaky upstream and gets 503'd under load). See #88532 follow-up.
        if not model and runtime_kwargs.get("provider"):
            with suppress(Exception):
                _fallback_model = None
                _cfg_model = (user_config or {}).get("model") if isinstance(user_config, dict) else None
                if isinstance(_cfg_model, dict):
                    _fallback_model = _cfg_model.get("default") or _cfg_model.get("model")
